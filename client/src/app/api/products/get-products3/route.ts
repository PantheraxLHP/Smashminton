...
export async function GET(request: Request) {
    try {
        ...
        const response = await fetch(
            `${process.env.SERVER}/api/v1/product-types/${productTypeId}/products/v3?page=${page}&pageSize=${pageSize}${productFilterValues ? `&productfiltervalue=${productFilterValues}` : ''}`,
            {
                headers: {
